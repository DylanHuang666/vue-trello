    ...
    <div class="list" ref="list">
      <div class="list-header" ref="listHeader">
        <textarea
          ...
          @blur="editListName"
        >{{data.name}}</textarea>
        <div class="extras-menu delete" @mousedown.prevent>
          <span @click="deleteList">删除列表</span>
        </div>
      </div>
...
export default {
  ...
  methods: {
      ...
      if (this.drag.isDown) {
        if (this.drag.isDrag) {
          ...
          if (e.path.includes(this.$refs.newBoardListName)) {
            this.$refs.newBoardListName.focus();
          }
        }
    ...
    //删除列表
    async deleteList(){
      await this.$store.dispatch("list/deleteList", this.data.id);
